...
import { IntegrationConnectionService, ConnectionStatus } from './IntegrationConnectionService';
...
export class OAuthFlowManagerAdapter {
  ...
  private statusCache: Map<string, { status: ConnectionStatus; timestamp: number }> = new Map();
  ...
  async getIntegrationStatus(
    provider: 'google' | 'box' | 'dropbox' | 'apple_connect'
  ): Promise<{
    connected: boolean;
    accountEmail?: string;
    accountName?: string;
    ...
    try {
      ...
      const organizationId = (auth.currentUser as any).organizationId || 
                            (auth.currentUser as any).claims?.organizationId;
      ...
      const status = await IntegrationConnectionService.getConnectionStatus(
        db,
        organizationId,
        provider === 'apple_connect' ? 'apple' : provider
      );
   ...
   */
  private formatStatus(status: ConnectionStatus): {
    connected: boolean;
    accountEmail?: string;
    accountName?: string;
    expiresAt?: string;
  ...
  async initiateOAuth(
    ...
    try {
      ...
      return result.data as { authUrl: string; state: string };
    } catch (error) {
   ...
   * Clear status cache
   */
  clearStatusCache(provider?: 'google' | 'box' | 'dropbox' | 'apple_connect') {
    if (provider) {
   ...
   */
  async hasBoxDocument(): Promise<boolean> {
