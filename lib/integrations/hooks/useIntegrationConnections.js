 ...
 * ```
 */
export function useIntegrationConnections(organizationId) {
    ...
    useEffect(() => {
        ...
        providers.forEach(provider => {
            const unsub = IntegrationConnectionService.subscribeToConnection(db, organizationId, provider, (status) => {
    ...
    const connectProvider = useCallback((provider) => {
        ...
        IntegrationConnectionService.redirectToOAuthFlow(provider, currentUrl);
    }, []);
